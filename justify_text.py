...
def justify_word(list_of_words, line_len, word_in_lines, char_in_lines, space_def):
    while len(list_of_words) > 0:
        ...
        word_in_one_line = []
        while over_char < line_len:
            try:
                char_count += len(list_of_words[0]) + space_def
                word_in_one_line.append(list_of_words[0])
                list_of_words.popleft()
...
def justify_space(word_in_lines, char_in_lines, space_def, extra_space, extra_space_remain, line_len):
    for line in range(len(char_in_lines)):
        justify_line = ''
        if char_in_lines[line] != line_len:
            extra_space = space_def + ((line_len - char_in_lines[line]) // (len(word_in_lines[line]) - 1))
            extra_space_remain += ((line_len - char_in_lines[line]) % (len(word_in_lines[line]) - 1))
            temp_extra_space_remain = 0
            for word in range(len(word_in_lines[line]) -1):
                if extra_space_remain > 0:
                    word_in_lines[line][word] += ' '*extra_space + ' '
                    extra_space_remain -= 1
                    temp_extra_space_remain += 1
                else:
                    word_in_lines[line][word] += ' '*extra_space

            # Print final justify result
            justify_line = ''.join(word_in_lines[line])
...
def justify_text():
    ...
    extra_space = 0 # Spaces that can distributed evenly
    extra_space_remain = 0 # Spaces remain after distributing evenly
