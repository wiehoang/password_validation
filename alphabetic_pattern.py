...
def print_pattern(n):
    for row in range(1, 2 * n):
        ...
        # Calculate number of half-side dashes on each row
        num_of_dashes: int
        if row <= n:
        ...
        # Calculate number of half-left chars on each row
        num_of_hl_chars: int
        if row <= n:
            num_of_hl_chars = row
        else:
        ...
        char_val: int = 97 + n - 1  # 97 is ASCII code of starting letter 'a'
        ...
        for char in range(num_of_hl_chars):
        ...
        for char in range(num_of_hl_chars - 1):
            print(chr(upd_char_val), end = '-')
            upd_char_val += 1

        # Print half right dashes
        for space in range(num_of_dashes - 1):
            print('-', end = '')
