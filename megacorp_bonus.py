...
def compute_bonus(ees_loc):
    # Compare current number with the number on the left
    for i in range(1, len(ees_loc)): # Add 1-bonus if the number is higher
        if ees_loc[i] > ees_loc[i-1]:
            ees_b[i] += 1
        elif ees_loc[i] < ees_loc[i-1]:
            for j in range(i,0,-1): # Reverse a loop, if 2 numbers have the same value, add 1-bonus to the number on the left
